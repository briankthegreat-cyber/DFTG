...
npm run build:site      # build + prerender (what the deploy workflow runs)
...
npm test                # unit tests (anatomy layout, condition model, timeline, copy rules, SEO metadata)
npm run test:e2e        # Playwright: pages, navigation, widgets, WebGL explainer, titles/canonicals/structured data, 404, crawler files
npm run assets          # regenerate favicons, app icons, social share images and explainer posters into public/
npm run prerender       # bake each route's <head> into dist/ and write sitemap.xml, robots.txt, 404.html
...
A workflow at `.github/workflows/pages.yml` runs the unit tests, builds the site, prerenders every
route and deploys it on every push to `main`. One-time setup: repository **Settings -> Pages ->
Source: GitHub Actions**. The site then lives at `https://briankthegreat-cyber.github.io/DFTG/` and
the explainer at `https://briankthegreat-cyber.github.io/DFTG/embed.html`.

**Custom domain** (for example `dontfretthegut.org`), three steps, no code changes:

1. At your domain registrar, point the domain at GitHub Pages (an `A`/`AAAA` record for the apex or a
   `CNAME` for `www`, per [GitHub's guide](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site/managing-a-custom-domain-for-your-github-pages-site)).
2. In the repository, **Settings -> Secrets and variables -> Actions -> Variables**, add
   `SITE_DOMAIN` = `dontfretthegut.org`.
3. Under **Settings -> Pages**, enter the same domain and tick **Enforce HTTPS** once the
   certificate is issued.

The workflow then builds for the root path, writes the `CNAME` file, and every canonical link,
sitemap entry, share image and structured-data URL switches to the new domain automatically.

Netlify, Vercel, or Cloudflare Pages also work: build command `npm run build:site` with
`VITE_BASE=/` and `VITE_SITE_URL=https://your-domain`, output folder `dist`, "single page app"
fallback on.
...
## Search, sharing and crawlers

What the build produces for search engines, link previews and AI assistants, and where to change it:

| Item | Where it lives |
| --- | --- |
| Unique page titles and meta descriptions | `seo` in `src/site/data.ts`; applied per route by `useSeo` in `src/site/seo.ts` and baked into each page's static HTML by `scripts/prerender.mjs` |
| Canonical links, Open Graph and Twitter tags | `useSeo` (built from `VITE_SITE_URL`) |
| Social share images (1200 x 630) | `public/og/*.jpg`, generated by `npm run assets` from the page copy |
| Structured data (JSON-LD) | `src/site/seo.ts`: NGO organization, WebSite, BreadcrumbList, FAQPage, MedicalWebPage with citations, VideoObject for the explainer, ItemList for guides and products. The foundation has no public street address, so the organization is described as an `NGO` rather than a `LocalBusiness`; add `address` in `organizationSchema()` if that changes |
| Breadcrumbs and related links | `Breadcrumbs` and `RelatedLinks` in `src/site/components/`; related links per page in `related` in `data.ts` |
| Sources under every guide | `sources` on each guide and the IBD/IBS pages in `data.ts` |
| Custom 404 | `src/site/pages/NotFound.tsx` (`noindex`) plus `dist/404.html` for GitHub Pages |
| `sitemap.xml`, `robots.txt` | written by the prerender step with the deployed site URL |
| `llms.txt` | `public/llms.txt`, a plain-text summary for AI crawlers |
| Favicon, app icons, web manifest | `public/favicon.svg`, `public/*.png`, `public/site.webmanifest` |
| Alt text | posters and product art carry descriptive alt text or `aria-label`; decorative motion is `aria-hidden` |
| Bundle size | routes and the 3D explainer are lazy-loaded; Three.js only downloads on pages that show the explainer. Production source maps are off |
| No-JavaScript fallback | each prerendered page includes a `<noscript>` summary with the page's heading, description and navigation |

After editing copy or the palette, run `npm run assets` and commit the updated `public/` files.
...
- **Instagram and email.** Set `org.instagram` and `org.email` in `src/site/data.ts`, then run `npm run assets` so the share images and structured data pick them up.
- **Domain.** Add the `SITE_DOMAIN` repository variable (see Publishing) so canonicals and the sitemap use the real domain from the first crawl.
...
dftg-site/
  ...
  src/site/
    ...
    App.tsx                     Routes
    seo.ts                      Per-page metadata hook and JSON-LD builders
    components/                 SiteLayout (header, footer, bag), ui, sections, Breadcrumbs, RelatedLinks
  ...
  public/                       favicon, icons, manifest, robots.txt, llms.txt, og/ share images and posters
  scripts/                      prerender, generate-assets, serve-static, make-preview, screenshots, site-screenshots, render-video
  test/unit, test/e2e           node:test unit tests, Playwright browser tests
  embed/index.html              The committed single-file explainer (+ ibd-animation.mp4)
```
