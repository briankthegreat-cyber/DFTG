...
export interface CameraRig {
  ...
  /** Slow autonomous drift added on top of user orbit (ambient mode). */
  setDrift(yaw: number, pitch: number): void;
...
export function createCameraRig(camera: PerspectiveCamera): CameraRig {
  ...
  function applyOrbit(base: Vector3, center: Vector3, out: Vector3): void {
    ...
    offsetSpherical.theta += yaw + driftYaw;
    offsetSpherical.phi = clamp(offsetSpherical.phi + pitch + driftPitch, 0.15, Math.PI - 0.15);
