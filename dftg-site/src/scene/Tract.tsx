...
export function Tract() {
  const { curves, quality, sceneState, meshes, options } = useSceneContext();
  ...
  useFrame(({ clock }) => {
    ...
    const time = options.ambient ? s.time + clock.elapsedTime : s.time;
    for (const p of parts) updateTissueMaterial(p.material, s, time, peristalsis);
  });
