  ...
  return (
    <main className="relative">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[80vh] opacity-60">
        <Particles particleColors={['#b8934f', '#d9bb85']} particleCount={120} particleSpread={10} speed={0.05} particleBaseSize={70} alphaParticles className="h-full w-full" />
      </div>
      <Section className="relative pt-40 md:pt-48">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-6 text-ink/60">{brand.mission}</p>
        </div>
      </Section>
            ...
            {milestones.map((m, i) => (
              <Reveal key={m.label} delay={i * 0.1}>
                <li className="relative mb-8">
                  <span className="absolute -left-[41px] top-1.5 h-4 w-4 rounded-full border-2 border-gold bg-cream shadow-[0_0_20px_rgba(184,147,79,0.6)]" />
      ...
      <Section id="team" className="!pt-0">
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <FloatTitle text="The people who know your name." align="center" className="text-4xl md:text-6xl" />
          <p className="mt-6 text-ink/60">Physicians, clinicians, and a front desk that answers the phone.</p>
        </div>
        <div className="mt-14">
          <TeamGrid />
        </div>
